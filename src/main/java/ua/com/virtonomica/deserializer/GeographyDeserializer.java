...
public class GeographyDeserializer extends JsonDeserializer<GeographyWrapper> {
    ...
    public GeographyWrapper deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException, JsonProcessingException {
        ...
        final double city_salary ;
        final double city_education;

        if (jsonNode.get("salary")==null &&  jsonNode.get("education")==null){
            return new GeographyWrapper(
                    city_id, city_name, country_id, country_name, region_id, region_name
            );
        } else {
            city_salary = jsonNode.get("salary").asDouble();
            city_education = jsonNode.get("education").asDouble();
            return new GeographyWrapper(
